...
const Asfalto: React.FC<AsfaltoProps> = ({ onBack }) => {
  ...
  async function uploadPdf(ordemId: string): Promise<{
    ...
    const path = `asfalto/${ordemId}/os-pdf/${Date.now()}-${safeName}`;
  ...
  async function handleSave(continuarComCamposVazios = false) {
    ...
    try {
      ...
      const slaConfigCadastro = getSlaConfig("ASFALTO");
      ...
      await setDoc(ordemRef, {
        ...
        slaConfigVersao: 1,
        slaHoras: getSlaHorasPorServico("ASFALTO"),
        slaPausas: [],
      ...
      setResultMessage(
        anexoStatusFinal === "PENDENTE"
          ? "Ordem de serviço de Asfalto cadastrada. O PDF não foi enviado agora e ficou salvo na fila local de anexos pendentes para reenvio."
          : "Ordem de serviço de Asfalto cadastrada com sucesso."
      );
      setShowResultModal(true);
    } catch (error: unknown) {
      console.error(error);
    ...
    <section className="page-card">
      ...
      >
        ...
        <div className={`page-section mobile-form-panel ${mobileStep === "anexos" ? "is-active" : ""}`}>
          <h3>OS em PDF</h3>
          <p className="page-section-description">
            Anexe o PDF da ordem de serviço de Asfalto. Esse arquivo ficará vinculado ao cadastro e será aberto na Lista de OS.
          </p>
